...
void SV_PushRotate( edict_t *pusher, float movetime ) {
    ...
    for ( i = 0; i < 3; i++ ) amove[i] = pusher->v.avelocity[i] * movetime;
...
#endif
            SV_PushMove( ent,
                         movetime );  // advances ent->v.ltime if not blocked
    }
...
#ifdef QUAKE2
            VectorSubtract( ent->v.velocity, ent->v.basevelocity,
                            ent->v.velocity );
#endif
            break;
        ...
        if ( wasonground )
            if ( !( ent->v.health <= 0.0 && !SV_CheckBottom( ent ) ) ) {
                ...
                if ( speed ) {
                    ...
                    control =
                        speed < sv_stopspeed.value ? sv_stopspeed.value : speed;
