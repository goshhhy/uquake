...
void R_BuildLightMap( msurface_t *surf, byte *dest, int stride ) {
    ...
    if ( r_fullbright.value || !cl.worldmodel->lightdata ) {
        for ( i = 0; i < size; i++ ) blocklights[i] = 255 * 256;
        goto store;
    }
...
void R_DrawSequentialPoly( msurface_t *s ) {
    ...
    if ( gl_mtexable ) {
        ...
        for ( i = 0; i < p->numverts; i++, v += VERTEXSIZE ) {
            ...
            nv[0] = v[0] + 8 * sin( v[1] * 0.05 + realtime ) *
                               sin( v[2] * 0.05 + realtime );
            nv[1] = v[1] + 8 * sin( v[0] * 0.05 + realtime ) *
                               sin( v[2] * 0.05 + realtime );
            nv[2] = v[2];
...
void GL_CreateSurfaceLightmap( msurface_t *surf ) {
    ...
    base = lightmaps + surf->lightmaptexturenum * lightmap_bytes * BLOCK_WIDTH *
                           BLOCK_HEIGHT;
...
void GL_BuildLightmaps( void ) {
    ...
    for ( i = 0; i < MAX_LIGHTMAPS; i++ ) {
        ...
        glTexImage2D(
            GL_TEXTURE_2D, 0, lightmap_bytes, BLOCK_WIDTH, BLOCK_HEIGHT, 0,
            gl_lightmap_format, GL_UNSIGNED_BYTE,
