    ...
    while ( 1 ) {
                ...
                switch ( p->mtype ) {
                    case MTYPE_RELIABLE:
                        ...
                        if ( p->sequence == p->sock->receiveSequence ) {
                            p->sock->receiveSequence =
                                ( p->sequence + 1 ) & 0xff;
                            p->sock->receiveMessageLength += p->lengthFound;
                        } else
                            Con_DPrintf(
                                "Serial: reliable out of order; got %u wanted "
                                "%u\n",
        ...
        switch ( p->currState ) {
            ...
            case STATE_LENGTH2:
                p->lengthStated += b;
                CRC_ProcessByte( &p->crcValue, b );
                if ( p->mtype == MTYPE_RELIABLE &&
                     p->lengthStated > MAX_MSGLEN ) {
                    p->currState = STATE_ABORT;
                    ...
                    if ( p->mtype < MTYPE_ACK ) {
                        *(short *)&p->sock
                             ->receiveMessage[p->sock->receiveMessageLength +
                                              1] = p->lengthStated;
                        p->lengthFound += 2;
                    }
                }
