...
void SubdividePolygon( int numverts, float *verts ) {
    ...
    for ( i = 0; i < 3; i++ ) {
        ...
        m = gl_subdivide_size.value *
            floor( m / gl_subdivide_size.value + 0.5 );
        if ( maxs[i] - m < 8 )
            continue;
        if ( m - mins[i] < 8 )
            continue;
...
void EmitWaterPolys( msurface_t *fa ) {
    ...
    for ( p = fa->polys; p; p = p->next ) {
        ...
        for ( i = 0, v = p->verts[0]; i < p->numverts; i++, v += VERTEXSIZE ) {
            ...
            s = os +
                turbsin[(int)( ( ot * 0.125 + realtime ) * TURBSCALE ) & 255];
            s *= ( 1.0 / 64 );

            t = ot +
                turbsin[(int)( ( os * 0.125 + realtime ) * TURBSCALE ) & 255];
            t *= ( 1.0 / 64 );
...
void R_DrawSkyChain( msurface_t *s ) {
    ...
    for ( fa = s; fa; fa = fa->texturechain ) EmitSkyPolys( fa );
...
void LoadTGA( FILE *fin ) {
    ...
    if ( targa_header.colormap_type != 0 ||
        ...
        Sys_Error(
            "Texture_LoadTGA: Only 32 or 24 bit images supported (no "
            "colormaps)\n" );
    ...
    if ( targa_header.id_length != 0 )
        fseek( fin, targa_header.id_length,
               SEEK_CUR );  // skip TARGA image comment
