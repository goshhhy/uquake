...
#define RETURN_EDICT( e ) \
    ( ( (int *)pr_globals )[OFS_RETURN] = EDICT_TO_PROG( e ) )
...
void PF_precache_file( void ) {  // precache_file is only used to copy files
...
void PF_changelevel( void ) {
    ...
    if ( (int)pr_global_struct->serverflags &
         ( SFL_NEW_UNIT | SFL_NEW_EPISODE ) )
...
void PF_WaterMove( void ) {
    ...
    if ( watertype == CONTENT_LAVA ) {  // do damage
        ...
        if ( !( flags & ( FL_IMMUNE_SLIME + FL_GODMODE ) ) )
            if ( self->v.dmgtime < sv.time &&
                 self->v.radsuit_finished < sv.time ) {
                self->v.dmgtime = sv.time + 1.0;
    ...
    if ( !( flags & FL_INWATER ) ) {
        ...
        if ( watertype == CONTENT_WATER )
            ...
            //ATTN_NORM);
            SV_StartSound( self, CHAN_BODY, "player/inh2o.wav", 255,
                           ATTN_NORM );
