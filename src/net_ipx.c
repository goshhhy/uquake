...
#define IPXBUFFERS \
    ( ( LOWMEMSIZE - LOWMEMSAVE ) / sizeof( ipx_lowmem_buffer_t ) )
...
int IPX_Connect( int handle, struct qsockaddr *addr ) {
    ...
    if ( IPX_GetLocalTarget(
             &ipxaddr, lma->socketbuffer[handle][0].ecb.immediateAddress ) !=
         0 ) {
...
tryagain:
    ...
    copylen = ntohs( rcvbuf->header.length ) -
              ( sizeof( int ) + sizeof( IPXheader ) );
    if ( len < copylen )
