...
void Draw_Character( int x, int y, int num ) {
    ...
    if ( r_pixbytes == 1 ) {
        ...
        pusdest = (unsigned short *)( (byte *)vid.conbuffer +
                                      y * vid.conrowbytes + ( x << 1 ) );
...
void Draw_Pic( int x, int y, qpic_t *pic ) {
    ...
    if ( ( x < 0 ) || ( x + pic->width > (int)vid.width ) || ( y < 0 ) ||
        ...
        Sys_Error( "Draw_Pic: bad coordinates %i.%i", x, y );
    }
...
void Draw_TransPic( int x, int y, qpic_t *pic ) {
    ...
    if ( x < 0 || (unsigned)( x + pic->width ) > vid.width || y < 0 ||
        ...
        Sys_Printf( "Draw_TransPic: bad coordinates %i.%i", x, y  );
    }
...
#elif defined( X11 )
    sprintf( ver, "(X11 Quake %2.2f) %4.2f", (float)X11_VERSION,
             (float)VERSION );
    dest = conback->data + 320 * 186 + 320 - 11 - 8 * strlen( ver );
