...
#define MP( a )                                                            \
    (int)( ( current_pixel_multiply ) ? ( ( a ) / current_pixel_multiply ) \
                                      : ( a ) )
...
void VID_Init( unsigned char *palette ) {
    ...
    // setup attributes for main window
    {
        ...
        tmpcmap =
            XCreateColormap( x_disp, XRootWindow( x_disp, x_visinfo->screen ),
                             x_vis, AllocNone );
...
void GetEvent( void ) {
        ...
        case MotionNotify:
            ...
            if ( _windowed_mouse.value ) {
                ...
                XSelectInput( x_disp, x_win,
                              x_std_event_mask & ~PointerMotionMask );
    ...
    if ( old_windowed_mouse != _windowed_mouse.value ) {
        ...
        if ( !_windowed_mouse.value ) {
            ...
            /* grab the pointer */
            XGrabPointer( x_disp, x_win, True, 0, GrabModeAsync, GrabModeAsync,
...
XilImage drain_renderpipeline( XilImage old ) {
    ...
    if ( old )
        if ( read( render_pipeline[1], &new, sizeof( new ) ) !=
             sizeof( new ) ) {
...
XilImage sched_update( XilImage image ) {
    ...
    if ( write( render_pipeline[1], &image, sizeof( image ) ) !=
         sizeof( image ) )
...
void *update_thread() {
    ...
    while ( read( render_pipeline[0], &image, sizeof( image ) ) ==
            sizeof( image ) ) {
